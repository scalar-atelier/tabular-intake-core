...
const messages: Record<Language, Record<string, string>> = {
  ko: {
    ...
    title: "엑셀 명단을 올리면, 쓸 수 있는 목록과 확인할 목록으로 나눠드려요",
    ...
    resultEyebrow: "기기 안에서 정리 완료", resultTitle: "이제 두 목록만 확인하세요",
    resultLead: "바로 쓸 수 있는 명단과 사람이 확인할 명단을 따로 만들었습니다.",
    downloadNormalized: "바로 쓸 명단 받기", downloadReview: "확인할 명단 받기", downloadNote: "엑셀에서 바로 열 수 있도록 만든 파일입니다.",
    previewTitle: "명단 내용 미리보기", normalizedPreview: "바로 쓸 명단", reviewPreview: "확인할 명단",
    ...
    summary_normal: "바로 써도 되는 명단", summary_information_review: "빠진 정보를 채울 명단", summary_duplicate_candidate: "같은 사람인지 확인할 명단",
  ...
  en: {
    ...
    title: "Turn a spreadsheet roster into a ready list and a check list",
    ...
    resultEyebrow: "Finished on this device", resultTitle: "Check these two lists", resultLead: "We separated records you can use now from records a person should check.",
    downloadNormalized: "Download ready list", downloadReview: "Download check list", downloadNote: "These files include a BOM so Excel can open them directly.",
    previewTitle: "Preview the lists", normalizedPreview: "Ready list", reviewPreview: "Check list", technical: "Technical details and processing proof",
    ...
    summary_normal: "Ready to use", summary_information_review: "Needs missing information", summary_duplicate_candidate: "Check if this is the same person",
  ...
  ja: {
    language: "言語", eyebrow: "インストールなしで名簿整理", title: "表計算の名簿を、すぐ使う一覧と確認する一覧に分けます",
    ...
    resultEyebrow: "端末内で完了", resultTitle: "2つの一覧を確認してください", resultLead: "すぐ使える名簿と、人が確認する名簿を分けました。",
    downloadNormalized: "すぐ使う名簿を保存", downloadReview: "確認する名簿を保存", downloadNote: "Excelで直接開けるファイルです。",
    previewTitle: "名簿をプレビュー", normalizedPreview: "すぐ使う名簿", reviewPreview: "確認する名簿", technical: "技術情報と処理証明ファイル",
    ...
    summary_normal: "すぐ使える名簿", summary_information_review: "不足情報を補う名簿", summary_duplicate_candidate: "同じ人か確認する名簿",
...
function renderResult(result: IntakeOutput): void {
  ...
  const readyName = language === "ko" ? "정리된-명단.csv" : language === "ja" ? "整理済み名簿.csv" : "ready-roster.csv";
...
function showStep(step: number, focus = true): void {
  ...
  if (focus) requestAnimationFrame(() => document.querySelector<HTMLElement>(`.demo-step[data-step="${currentStep}"] h2`)?.focus({ preventScroll: true }));
}
